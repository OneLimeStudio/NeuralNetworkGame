...
def Spawner():
    x = random.sample([15,15+15+15+TRAIN_WIDTH,2*(15+15+7.5+TRAIN_WIDTH),3*(TRAIN_WIDTH)+15*7],2)
    return x
...
class Subway:    
    def __init__(self):
        ...
        x = random.sample([15,15+15+15+PLAYER,2*(15+15+7.5+PLAYER),3*(PLAYER)+15*7],1)
    ...
    def train_ai(self,genome,config):
        ...
        while run:
            self.clock.tick(60)
            self.window.fill((0,0,0))
            pygame.draw.line(self.window,(0,255,0),(SCREEN_WIDTH/4,0 ),(SCREEN_WIDTH/4, SCREEN_HEIGHT))
            pygame.draw.line(self.window,(0,255,0),(2*SCREEN_WIDTH/4,0 ),(2*SCREEN_WIDTH/4, SCREEN_HEIGHT))
            pygame.draw.line(self.window,(0,255,0),(3*SCREEN_WIDTH/4,0 ),(3*SCREEN_WIDTH/4, SCREEN_HEIGHT))
            ...
            pygame.display.update()
            if(self.Player.x != self.train1.x or self.Player.x != self.train2.x):
...
def run_neat(config):
    p = neat.Checkpointer.restore_checkpoint('neat-checkpoint-145')
